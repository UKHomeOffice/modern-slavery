export const NRM_SELECTORS = {
  START_HOME_BUTTON: '#gds-button',
  START_REPORT: '#gov-grid-row-content > div > form > div > div > input.govuk-button',
  CONTINUE_BUTTON: '#gov-grid-row-content form input.govuk-button, #gov-grid-row-content form button.govuk-button:not(.govuk-button--secondary)',
  REFERENCE_INPUT: '#reference',
  EMAIL_INPUT: '#user-email',
  ORGANISATION_INPUT: '#user-organisation',
  PV_BIRTHPLACE: '#birthplace',
  PV_FAMILY: '#family',
  PV_EDUCATION: '#education',
  PV_EMPLOYMENT_HISTORY: '#employment-history',
  EXPLOITATION_TAKE_PLACE: '#when-did-the-exploitation-take-place',
  POTENTIAL_VICTIM_EXPLOITATIVE_SITUATION_NO: '#potential-victim-exploitative-situation-multiple-no',
  HOW_DID_EXPLOITATION_START: '#how-did-the-exploitation-start',
  WERE_TAKEN_SOMEWHERE_YES: '#were-they-taken-somewhere-by-their-exploiter-yes',
  WERE_TAKEN_SOMEWHERE_DETAILS: '#were-they-taken-somewhere-by-their-exploiter-journey-details',
  WHAT_WERE_THEY_REQUIRED_TO_DO: '#what-were-they-required-to-do',
  HOW_WERE_THEY_TREATED: '#how-they-were-treated',
  WHY_THEY_STAYED: '#why-they-stayed',
  STILL_IN_EXPLOITATIVE_SITUATION_YES_OPTION: '#still-in-exploitative-situation-yes',
  WHAT_IS_KEEPING_THEM_IN_SITUATION: '#what-is-keeping-them-in-situation',
  WHY_DID_THEY_LEAVE: '#how-why-did-they-leave-the-situation',
  WHEN_LAST_CONTACT_LAST_WEEK: '#when-last-contact-within-the-last-week',
  DETAILS_LAST_CONTACT: '#details-last-contact',
  FIRST_CHANCE_TO_REPORT_NO: '#is-this-the-first-chance-to-report-no',
  WHY_REPORT_NOW: '#why-report-now',
  MODERN_SLAVERY_INDICATORS_YES_OPTION: '#modern-slavery-indicators-yes',
  MODERN_SLAVERY_INDICATORS_DETAILS: '#modern-slavery-indicators-details',
  WHY_ARE_YOU_MAKING_REFERRAL: '#professional-insight',
  INTERVIEW_CARRIED_OUT: '#where-how-interview-carried-out',
  ARE_OTHERS_INVOLVED_YES: '#are-others-involved-yes',
  ARE_OTHERS_INVOLVED_DETAILS: '#are-others-involved-details',
  EVIDENCE_OF_DISHONESTY_YES: '#evidence-of-dishonesty-yes',
  EVIDENCE_OF_DISHONESTY_DETAILS: '#evidence-of-dishonesty-details',
  EVIDENCE_TO_SUBMIT: '#what-evidence-you-will-submit',
  LOCATION_ENGLAND_OPTION: '#fr-location-england',
  PV_UNDER_AGE_NO_OPTION: '#pv-under-age-no',
  PV_UNDER_AGE_AT_TIME_OF_EXPLOITATION_NO_OPTION: '#pv-under-age-at-time-of-exploitation-no',
  PV_UNDER_AGE_YES_OPTION: '#pv-under-age-yes',
  LOCAL_AUTHORITY_NAME: '#local-authority-contacted-about-child-local-authority-name',
  LOCAL_AUTHORITY_PHONE: '#local-authority-contacted-about-child-local-authority-phone',
  LOCAL_AUTHORITY_EMAIL: '#local-authority-contacted-about-child-local-authority-email',
  EXPLOITED_IN_UK_OPTION: '#where-exploitation-happened-uk',
  EXPLOITED_IN_UK_CITY_INPUT: '#where-exploitation-happened-uk-city-1',
  CURRENT_PV_LOCATION_UK_CITY: '#current-pv-location-uk-city',
  CURRENT_PV_LOCATION_UK_REGION: '#current-pv-location-uk-region',
  WHO_EXPLOITED_PV: '#who-exploited-pv',
  EXPLOITERS_LOCATION_YES: '#exploiters-location-yes',
  ARE_EXPLOITERS_IN_THE_UK_YES: '#are-exploiters-in-the-uk-yes',
  EXPLOITERS_CURRENT_LOCATION_DETAILS: '#exploiters-current-location-details',
  HOW_WERE_THEY_EXPLOITED_FORCED_WORK_OPTION: '#types-of-exploitation-forced-to-work',
  ANY_OTHER_PVS_NO_OPTION: '#any-other-pvs-no',
  FUTURE_EXPLOITATION_CONCERNS_YES: '#future-exploitation-concerns-yes',
  FUTURE_EXPLOITATION_REASONS: '#future-exploitation-reasons',
  PV_HAS_CRIME_REFERENCE_NUMBER_NO_OPTION: '#reported-to-police-no',
  COOPERATION_WITH_AUTHORITIES_NO_OPTION: '#authorities-cooperation-no',
  REFER_CASE_TO_NRM_YES_OPTION: '#pv-want-to-submit-nrm-yes',
  REFER_CASE_TO_NRM_NO_OPTION: '#pv-want-to-submit-nrm-no',
  DOES_PV_NEED_SUPPORT_YES_OPTION: '#does-pv-need-support-yes',
  PV_PHONE_NUMBER_YES_OPTION: '#pv-phone-number-yes',
  PV_PHONE_NUMBER_ALTERNATIVE_OPTION: '#pv-phone-number-alternative',
  NO_CONTACT_DETAILS_INPUT: '#no-contact-details',
  PV_NAME_FIRST_NAME: '#pv-name-first-name',
  PV_NAME_LAST_NAME: '#pv-name-last-name',
  PV_GENDER_MALE_OPTION: '#pv-gender-male',
  DOES_PV_HAVE_CHILDREN_NO_OPTION: '#does-pv-have-children-no',
  PV_NATIONALITY: '#pv-nationality',
  INTERPRETER_NO_OPTION: '#pv-interpreter-requirements-no',
  COMMUNICATION_AID_NO_OPTION: '#pv-other-help-with-communication-no',
  HO_REFERENCE_NO_OPTION: '#pv-ho-reference-no',
  WHO_CONTACT_PV_OPTION: '#who-contact-potential-victim',
  PV_CONTACT_DETAILS_EMAIL_OPTION: '#pv-contact-details-email',
  PV_CONTACT_DETAILS_EMAIL_INPUT: '#pv-contact-details-email-input',
  PV_CONTACT_DETAILS_EMAIL_SAFE_OPTION: '#pv-contact-details-email-check',
  PV_CONTACT_DETAILS_POST_OPTION: '#pv-contact-details-post',
  PV_CONTACT_DETAILS_STREET_INPUT: '#pv-contact-details-street',
  PV_CONTACT_DETAILS_TOWN_INPUT: '#pv-contact-details-town',
  PV_CONTACT_DETAILS_COUNTY_INPUT: '#pv-contact-details-county',
  PV_CONTACT_DETAILS_POSTCODE_INPUT: '#pv-contact-details-postcode',
  PV_CONTACT_DETAILS_POST_SAFE_OPTION: '#pv-contact-details-post-check',
  PV_PHONE_NUMBER_NO_OPTION: '#pv-phone-number-no',
  FR_DETAILS_FIRST_NAME_INPUT: '#fr-details-first-name',
  FR_DETAILS_LAST_NAME_INPUT: '#fr-details-last-name',
  FR_DETAILS_ROLE_INPUT: '#fr-details-role',
  FR_DETAILS_PHONE_INPUT: '#fr-details-phone',
  FR_ALTERNATE_CONTACT_EMAIL_INPUT: '#fr-alternative-contact',
} as const;
