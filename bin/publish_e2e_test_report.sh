...
jq -e . "$RELEASE_RESPONSE_FILE" >/dev/null 2>&1 || (echo "Release API returned non-JSON response" && sed -n '1,30p' "$RELEASE_RESPONSE_FILE" && rm -f "$RELEASE_RESPONSE_FILE" && exit 1)
RELEASE_ID="$(jq -r '.id // empty' "$RELEASE_RESPONSE_FILE")"
test -n "$RELEASE_ID" || (echo "Release response missing id" && sed -n '1,30p' "$RELEASE_RESPONSE_FILE" && rm -f "$RELEASE_RESPONSE_FILE" && exit 1)
...
jq -e . "$ASSET_RESPONSE_FILE" >/dev/null 2>&1 || (echo "Asset upload returned non-JSON response" && sed -n '1,30p' "$ASSET_RESPONSE_FILE" && rm -f "$ASSET_RESPONSE_FILE" && exit 1)
...
echo "$GH_COMMENT_RESPONSE" | jq -e . >/dev/null 2>&1 || (echo "GitHub comment API returned non-JSON response" && echo "$GH_COMMENT_RESPONSE" && exit 1)
